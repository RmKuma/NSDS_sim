...
{
#define COPYDATA(DEST,SRC,SIZE) if(MQSimulator->Is_integrated_execution_mode()) {DEST = new char[SIZE]; memcpy(DEST, SRC, SIZE);} else DEST = SRC;
#define DELETE_REQUEST_NVME(REQ) \
	delete (Submission_Queue_Entry*)REQ->IO_command_info; \
	if(MQSimulator->Is_integrated_execution_mode())\
